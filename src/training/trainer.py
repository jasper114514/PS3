...
def train(datasets, args):
    ...
    for epoch in range(args.max_epochs):
        ...
        train_results = train_loop_survival(model, datasets['train'], optimizer, lr_scheduler, loss_fn,
                                            print_every=args.print_every, accum_steps=args.accum_steps, process_text=args.process_text)
        ...
        # if epoch % 10 == 0:
            ...
            val_results, _ = validate_survival(model, datasets['test'], loss_fn,
                                                   print_every=args.print_every, verbose=True, process_text=args.process_text)

            ### Check Early Stopping (Optional)
        if early_stopper is not None:
            if args.es_metric == 'loss':
                score = train_results['loss']
    ...
    for k, loader in datasets.items():
        ...
        results[k], dumps[k] = validate_survival(model, loader, loss_fn, print_every=args.print_every,
                                                     dump_results=True, return_attn=return_attn, verbose=False, process_text=args.process_text)
...
def train_loop_survival(model, loader, optimizer, lr_scheduler, loss_fn=None,
                        print_every=50, accum_steps=32, process_text=False):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if process_text:
            out, log_dict = model(data, omics, text, attn_mask=attn_mask, label=label, censorship=censorship, loss_fn=loss_fn)
        else:
